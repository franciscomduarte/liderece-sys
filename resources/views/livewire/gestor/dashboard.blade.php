...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="float-in bg-white rounded-2xl px-5 py-4 shadow-[0_12px_40px_rgba(23,28,31,0.06)] ring-1 ring-black/[0.04]" style="animation-delay:200ms">
            ...
            <p class="text-[#727785] text-xs mt-1">Contestações pendentes</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="float-in bg-white rounded-2xl shadow-[0_12px_40px_rgba(23,28,31,0.06)] ring-1 ring-black/[0.04] overflow-hidden" style="animation-delay:300ms">
            <div class="px-5 py-4 border-b border-[#eaeef2] flex items-center justify-between">
                <h3 class="font-['Manrope'] font-bold text-sm text-[#171c1f]">Contestações para responder</h3>
                @if($stats['contestacoes_pendentes'] > 0)
                <a href="{{ route('gestor.contestacoes') }}" class="text-xs font-semibold text-[#0058be] hover:underline">Ver tudo</a>
                @endif
            </div>
            @if($contestacoes->isNotEmpty())
            <ul class="divide-y divide-[#eaeef2]">
                @foreach($contestacoes as $c)
                <li class="px-5 py-3.5">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <p class="text-sm font-semibold text-[#171c1f] truncate">{{ $c->servidor->nome }}</p>
                            <p class="text-xs text-[#727785] truncate">{{ $c->avaliacao->competencia->nome }}</p>
                        </div>
                        <span class="shrink-0 text-xs text-[#727785]">{{ $c->created_at->diffForHumans() }}</span>
                    </div>
                    <p class="text-xs text-[#424754] mt-1.5 line-clamp-2">{{ $c->justificativa }}</p>
                </li>
                @endforeach
            </ul>
            @else
            <div class="px-5 py-10 text-center">
                <span class="material-symbols-outlined text-3xl text-[#c2c6d6]">gavel</span>
                <p class="text-sm text-[#727785] mt-2">Nenhuma contestação pendente.</p>
            </div>
            @endif
        </div>
